...
- Blueprint 10 — Admin Control Center & Content Operations (NEXT, per user). Decisions locked: add a NEW "Home Intelligence Ops" section INSIDE existing /admin (reuse audit + feature-flag infra); FULL 5-role RBAC (Super/Product/Support/Content/Finance) — this is an AUTH change, MUST route through integration_expert before coding; Content Templates (draft→review→published→archived, versioning + rollback, Homie uses published as approved guidance context) is a must-have. Data models per blueprint: AdminUser, AdminPermission, FeatureFlag, ContentTemplate, SafetyEscalation, SupportTicket, SupportMessage, AdminAuditLog. Analytics events listed in spec. Do NOT view private docs/conversation content unless explicitly granted + audit-logged.
- Blueprint 07 — Tool/Material/Supply Inventory (central chat front-door using room/asset/project/doc context; context selector; photo ID; conversation→approved records with user approval; history; hi_conversation_*).
